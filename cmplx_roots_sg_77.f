...
! _2_                     CMPLX_ROOTS_5                             !
!-------------------------------------------------------------------!
      subroutine cmplx_roots_5
     * (roots, first_3_roots_order_changed, poly, polish_only)
      implicit none
     ...
     *                            roots(degree  ),poly2)
          ...
          if((root4.lt.degree-1).or.(root5.lt.degree-1)) then
            ...
            if(go_to_robust.gt.0)then
              ...
              i2=degree
              do i=1,degree
                if((i.ne.root4).and.(i.ne.root5))then
                  roots(i2)=roots(i)
                  i2=i2-1
                endif
...
!-------------------------------------------------------------------!
     ...
     *           (poly, degree, root, iter, success)
      ...
      double precision FRAC_JUMPS(FRAC_JUMP_LEN)
      double precision pi
      parameter(pi = 3.141592653589793d0)
      double precision faq ! jump length
      double precision FRAC_ERR
